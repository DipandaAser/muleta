<script lang="ts">
	...
	interface Props {
		jobs: Job[]
		loading: boolean
		/** Queue name, used to build the per-row link to the job detail route. */
		queueName: string
	}
	...
	function onRowClick(e: MouseEvent, id: string): void {
		// Let the browser handle modified clicks on the <a> inside the row.
...
<div class="overflow-x-auto w-full rounded-lg border border-base-300 bg-base-100">
	<table class="table table-sm">
		...
		<tbody>
			...
			{:else}
				{#each jobs as j (j.id)}
					<tr
						class="hover:bg-base-200/60 group text-[12.5px] cursor-pointer"
						onclick={(e) => onRowClick(e, j.id)}
					>
						...
						<td class="font-medium truncate">
							<a href={hrefFor(j.id)} class="hover:underline" onclick={(e) => e.stopPropagation()}>
								{j.name}
							</a>
						</td>
